<template>
	<view class="waterfall-item" @tap="onTap">
		<image :src="params.url" mode="widthFix" @load="emitHeight" @error="emitHeight" style="border-radius: 10px;"></image>
		<view class="content">
			<view>{{params.title}}</view>
			<view class="money">时间:{{params.time}}</view>
			<view style="margin: 0 0 8rpx 0;">
				<text class="label">{{params.label}}</text>
			</view>
			<view class="shop-name">{{params.shop=='2'?'捡到物品':'丢失物品'}}</view>
		</view>
	</view>
</template>
