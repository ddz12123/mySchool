<template>
	<view class="container flex1" :style="{
		paddingBottom: showMoreTool ? '400rpx' : '500rpx'
	}">
		...
		<view class="tool-view">
			...
			<view class="setting-layer" v-if="showSettingLayer">
				<view class="single" @click="release(2)">
					<jinIcon class="icon" type="&#xe639;"></jinIcon>
					<view>立即发布</view>
				</view>
				<view class="single" @click="release(0)">
					<jinIcon class="icon" type="&#xe655;"></jinIcon>
					<view>存为草稿</view>
				</view>
			</view>
					...
					success: async (res) => {
						...
						for (let temp of tempFilePaths) {
							...
							await uni.uploadFile({
								url: this.$myHttp.baseUrl + "/upload/uploads",
								filePath: temp,
								...
								success: res => {
									...
									this.editorCtx.insertImage({
										 src:this.$myHttp.baseUrl+"/public/image/upload/"+path,  // 此处需要将图片地址切换成服务器返回的真实图片地址
										//src: temp,
										alt: '图片',
										success: function(e) {}
									});
